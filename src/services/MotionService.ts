...
class MotionService {
     ...
     */
     public isInMotion(data: DataPoint[]): boolean {
        ...
        const minTimeDifference = 30; // Intervalo de tempo mínimo em segundos para considerar o objeto parado (exemplo: 60 segundos)
        const minSpeed = 0.1; // Limite mínimo de velocidade em m/s
        ...
        if (distance > threshold && timeDifference >= minTimeDifference && (speed > minSpeed)) {
            return true;
        }
        return false;
    }
